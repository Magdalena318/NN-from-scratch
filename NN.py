...
class NeuralNetwork:
    ...
    def __init__(self, c, w, stride, epochs, learning_rate, k, directory_path, X_train, Y_train, X_test, Y_test,
        ...
        # Default l2Regularization value for back propagation
        self.l2Regularization = 0.8
    ...
    def back_propagation(self, cBywVector, c, w, k, learning_rate, Y):
        ...
        for i in range(0, c):
            temp = partial_derivative[1][i] * learning_rate * self.l2Regularization
            for j in range(0, c):
    ...
    def train(self, c, w, stride, epochs, learning_rate, k, X_train, Y_train, row_train, column_train):
        ...
        for e in range(0, epochs):
            ...
            for rr in range(0, row_train):
                ...
                for ww in range(0, column_train, stride):
                    ...
                    # Check if windows are of equal dimensions
                    if (cBywVector.shape != prevShape and ww > 0):
                        break
                    ...
                    if (e == epochs - 1):
                        print('Window Prediction: %d, Expected: %d' % (predicted_value, Y_train[rr]),
                              file=self.f_window_prediction)
            ...
            accuracy_per_epoch = accuracy_score(actual_vals_per_epoch, predicted_vals_per_epoch)
            ...
            if (e == epochs - 1):
                ...
                conf_matrix_train = confusion_matrix(actual_vals_per_epoch, predicted_vals_per_epoch)
    ...
    def test(self, c, w, stride, k, X_test, Y_test, row_test, column_test):
        ...
        for rr in range(0, row_test):
            most_common_prediction_row = []
            print('\nFor Row[%d]:' % (rr), file=self.f_window_prediction)
            for ww in range(0, column_test, stride):
                ...
                # Window prediction
                print('Window Prediction: %d, Expected: %d' % (predicted_value, Y_test[rr]),
                      file=self.f_window_prediction)
            ...
            actual_vals_per_row.append(Y_test[rr])
            predicted_vals_per_row.append(most_common_pred)
            sys.stdout.write(".")
            sys.stdout.flush()

        print("\n-----------Confusion Matrix for test-----------", file=self.f_log)
        conf_matrix_test = confusion_matrix(actual_vals_per_row, predicted_vals_per_row)
        print(conf_matrix_test, file=self.f_log)

        print("\n-------Accuracy Metrics for test-------", file=self.f_log)
        print("\n-------Accuracy Metrics for test-------")
        accuracy_most_common = common_correct_preds_num / row_test
        print("Most Common Accuracy: ", np.around(accuracy_most_common, decimals=2), file=self.f_log)
        print("Most Common Accuracy: ", np.around(accuracy_most_common, decimals=2))

        accuracy_overall = accuracy_score(actual_vals_per_window, predicted_vals_per_window)
        print("Overall Accuracy for entire dataset: ", (np.around(accuracy_overall, decimals=4)), file=self.f_log)
        print("Overall Accuracy for entire dataset: ", (np.around(accuracy_overall, decimals=4)))
